smallPhotos = document.getElementsByClassName('gallery__thumbnail')
...
closePhotoByOutsideClick = ->
  container = document.getElementsByClassName(bigPhotoVisible)[0]
...
anotherPhotoOnButtonClick = (button, which) -> 
  $(button[curPic]).off().on('click', ->
    bigPhotos[curPic].classList.remove(bigPhotoVisible)
    if(which === 'next')
